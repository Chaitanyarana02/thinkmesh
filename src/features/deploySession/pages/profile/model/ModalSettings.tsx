...
export default function ModalSettings({ isOpen, onClose }: { isOpen: boolean, onClose: any }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0D12109E]">
            <div className="relative z-50 bg-white rounded-xl p-5 w-[90%] max-w-md mx-auto shadow-xl">
                <button onClick={onClose} className="absolute top-2.5 right-3 text-gray-500 hover:text-black text-xl">
                ...
                <div className="space-y-3 mt-6">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="User"
                            className="w-full pl-10 pr-3 py-2 border rounded-md text-sm outline-none"
                        />
                    </div>
                    ...
                    <div className="border-2 border-[#28F2A1] rounded-md p-4 text-center text-sm text-gray-500">
                        <div className="text-gray-400 mb-2 text-xl flex w-full items-center justify-center"> <Icon iconName="upload" /></div>
                        <span className="text-[#28F2A1] text-xs font-medium cursor-pointer">Click to upload</span> or drag and drop<br/>
                        <span className="text-xs text-gray-400">supported formats: PNG, JPG or GIF</span>
                    </div>

                    <button className="w-full font-sans mt-4 bg-[#28F2A1] text-black font-bold py-2 rounded-md border-custom">
                        Save Info
                    </button>
                </div>
            </div>
