  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="text-2xl font-bold font-sansBold uppercase">My Profile</div>
        <div className="hidden md:block"><ConnectWallet type="logo" noText={true} /> </div>
      </div>
      <div className="flex gap-4 w-full">
        {!enable ? (<>
          <div className="flex flex-col gap-2 items-center  justify-center w-full h-[calc(100vh-230px)]  px-4 z-0">
            <p className="text-2xl font-mono font-semibold text-center ">Please connect <br className="md:hidden" />your wallet</p>
            <p className="text-md text-[#475467] font-mono text-center ">You need to connect your wallet to view your profile</p>
          </div>
        </>) : (
            <div className="mt-6 w-full rounded-2xl  bg-white p-0 md:p-6">
              <div className="flex flex-row items-start items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="text-white border border-[#28F2A1] bg-[#B04FF5] rounded-full font-sans w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center font-semibold text-lg sm:text-xl">
                    JS
                  </div>
                  <div className="flex flex-col gap-1">
                    <p className="text-md sm:text-xl font-semibold uppercase text-black">John Smith</p>
                    <p className="text-xs sm:text-md text-[#475467]">0x71C7...976EF356FGF</p>
                  </div>
                </div>
                <div className="ml-auto sm:ml-0 ">
                  <div onClick={()=>{setToggleSetting(true)}}>
                    <Icon iconName="setting" />
                  </div>
                </div>
              </div>
              <div className=" mt-6 pt-4 grid gap-3">
                <div className="flex flex-col md:flex-row py-4 justify-between md:justify-start border-b text-sm sm:text-base">
                  <span className="font-medium mb-2 md:mb-0 text-black md:w-[340px]">Name:</span>
                  <span className="text-[#475467]">John Smith</span>
                </div>
                <div className="flex flex-col md:flex-row py-4 justify-between md:justify-start border-b text-sm sm:text-base">
                  <span className="font-medium mb-2 md:mb-0 text-black md:w-[340px]">Email Address:</span>
                  <span className="text-[#475467]">johnsmith@example.com</span>
                </div>
                <div className="flex flex-col md:flex-row py-4 justify-between md:justify-start text-sm sm:text-base">
                  <span className="font-medium mb-2 md:mb-0 text-black md:w-[340px]">Wallet Address:</span>
                  <span className="text-[#475467]">0x71C7...976EF356FGF</span>
                </div>
              </div>
              <ModalSettings isOpen={toggleSetting} onClose={()=>{setToggleSetting(false)}} />
            </div>
        )}
      </div>
    </div>
